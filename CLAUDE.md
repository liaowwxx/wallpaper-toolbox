...
RePKG ToolBox is a macOS-native SwiftUI app for browsing, managing, and extracting Wallpaper Engine wallpapers on macOS. It wraps the [RePKG](https://github.com/notscuffed/repkg) CLI (.NET) to extract `.pkg` files, manages metadata (ratings, collections, tags), and sets video/image wallpapers via a bundled native player.

| | |
|---|---|
| Package name | `RePKG-Native` |
| Bundle name | `RePKG ToolBox` |
| Target OS | macOS 26+ |
